...
parse_line_suppressions() {
  ...
  [ -f "$file" ] || return 0
  awk -v file="$file" '
    match($0, /sdk-review:[[:space:]]*ignore(\[[^]]*\])?/) {
...
parse_file_suppressions() {
  ...
  head -20 "$file" 2>/dev/null | awk -v file="$file" '
    match($0, /sdk-review-ignore-file:[[:space:]]*[a-zA-Z0-9_,-]+/) {
      m = substr($0, RSTART, RLENGTH)
      sub(/^sdk-review-ignore-file:[[:space:]]*/, "", m)
      n = split(m, arr, ",")
      for (j=1; j<=n; j++) {
        gsub(/^ +| +$/, "", arr[j])
        print file ":*:" arr[j]
...
is_suppressed() {
  ...
  # exact line + rule
  if grep -q "^${file}:${line}:${rule}$" "$supp_file"; then echo "true"; return; fi
  # exact line + wildcard
  if grep -q "^${file}:${line}:\*$" "$supp_file"; then echo "true"; return; fi
  # file + rule
  if grep -q "^${file}:\*:${rule}$" "$supp_file"; then echo "true"; return; fi
