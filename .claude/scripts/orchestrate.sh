...
$msg"
  # Anchorable only if file:line is a real entry in the added-line set.
  anchorable=false
  if [ -f "${ADDED_LINES_FILE:-/nonexistent}" ] && grep -Fxq "${file}:${line}" "$ADDED_LINES_FILE" 2>/dev/null; then
    anchorable=true
  fi
...
# The table + non-anchored findings section. Anchored findings appear as
# inline comments; non-anchored ones (PR body, missing files/dirs, commit
# metadata) are listed here so they are never invisible.
summary_table=$(jq -r '
  def status_icon:
    if . == "PASS" then "✅ PASS"
