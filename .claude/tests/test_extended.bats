...
@test "aggregate: SHADOW rule findings moved to shadow_findings, not posted" {
  [ -f "$SCRIPT_DIR/aggregate.sh" ] || skip "aggregate.sh not in this batch"
  tmpd=$(mktemp -d)
...
@test "apply-suppression: filters findings correctly" {
  [ -f "$SCRIPT_DIR/lib/apply-suppression.sh" ] || skip "apply-suppression not in this batch"
  supp_file=$(mktemp)
...
@test "orchestrate: empty diff produces summary with 0 findings" {
  [ -f "$SCRIPT_DIR/orchestrate.sh" ] || skip "orchestrate.sh not in this batch"
  tmpd=$(mktemp -d)
...
@test "check-secrets: does not fire on placeholder-looking test fixtures" {
  [ -f "$SCRIPT_DIR/check-secrets.sh" ] || skip "check-secrets.sh not in this batch"
...
@test "check-disclosure: PR body Closes #<issue_number> triggers DIS-07" {
  [ -f "$SCRIPT_DIR/check-disclosure.sh" ] || skip "check-disclosure.sh not in this batch"
  pr_body=$(mktemp)
