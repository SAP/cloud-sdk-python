...
from sap_cloud_sdk.auditlog_ng.client import AuditClient
from sap_cloud_sdk.auditlog_ng.config import AuditLogNGConfig, SCHEMA_URL
from sap_cloud_sdk.auditlog_ng.exceptions import ValidationError
...
@patch("sap_cloud_sdk.auditlog_ng.client.OTLPLogExporter")
@patch("sap_cloud_sdk.auditlog_ng.client.LoggerProvider")
def _make_mocked_client(mock_provider_cls, mock_exporter_cls, *, validate_side_effect=None):
    ...
    validate_patcher = patch(
        "sap_cloud_sdk.auditlog_ng.client.protovalidate.validate",
