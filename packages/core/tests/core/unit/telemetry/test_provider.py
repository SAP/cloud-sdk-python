...
from sap_cloud_sdk.telemetry._provider import (
    get_meter,
    shutdown,
    _setup_meter_provider,
)
from sap_cloud_sdk.telemetry.config import InstrumentationConfig
...
class TestGetMeter:
    ...
    def test_get_meter_returns_meter(self):
        ...
        import sap_cloud_sdk.telemetry._provider as provider_module

        # Reset global state
        provider_module._meter_provider = None
        provider_module._meter = None

        with patch('sap_cloud_sdk.telemetry._provider._setup_meter_provider') as mock_setup:
    ...
    def test_get_meter_when_provider_setup_fails(self):
        ...
        with patch('sap_cloud_sdk.telemetry._provider._setup_meter_provider', return_value=None):
...
class TestSetupMeterProvider:
    ...
    def test_setup_meter_provider_disabled(self):
        ...
        with patch('sap_cloud_sdk.telemetry._provider.get_config', return_value=config):
    ...
    def test_setup_meter_provider_success(self):
        ...
        with patch('sap_cloud_sdk.telemetry._provider.get_config', return_value=config):
            with patch('sap_cloud_sdk.telemetry._provider.Resource') as mock_resource_class:
                with patch('sap_cloud_sdk.telemetry._provider.OTLPMetricExporter') as mock_exporter_class:
                    with patch('sap_cloud_sdk.telemetry._provider.PeriodicExportingMetricReader') as mock_reader_class:
                        with patch('sap_cloud_sdk.telemetry._provider.MeterProvider') as mock_provider_class:
    ...
    def test_setup_meter_provider_with_valid_config(self):
        ...
        for config in test_configs:
            with patch('sap_cloud_sdk.telemetry._provider.get_config', return_value=config):
                with patch('sap_cloud_sdk.telemetry._provider.Resource'):
                    with patch('sap_cloud_sdk.telemetry._provider.OTLPMetricExporter') as mock_exporter:
                        with patch('sap_cloud_sdk.telemetry._provider.PeriodicExportingMetricReader'):
                            with patch('sap_cloud_sdk.telemetry._provider.MeterProvider') as mock_provider_class:
