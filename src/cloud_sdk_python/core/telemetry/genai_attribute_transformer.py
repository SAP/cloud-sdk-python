...
class GenAIAttributeTransformer(SpanExporter):
    ...
    def __init__(self, wrapped_exporter: SpanExporter):
        ...
        logger.info("GenAI attribute transformer initialized (minimal normalization enabled)")
    ...
    def export(self, spans: Sequence[ReadableSpan]) -> SpanExportResult:
        ...
        for span in spans:
            try:
                ...
                logger.debug(f"Error normalizing GenAI attributes for span {getattr(span, 'name', '<unknown>')}: {e}")
    ...
    def _normalize_attributes(self, span: ReadableSpan) -> None:
        ...
        if not any(k.startswith((self._TRACELOOP_PREFIX, self._OPENINFERENCE_PREFIX, self._GENAI_PROMPT_PREFIX, self._GENAI_COMPLETION_PREFIX)) for k in attrs.keys()):
            return
    ...
    def _map_llm_usage(self, attrs: MutableMapping[str, Any]) -> set:
        ...
        if self._LLM_USAGE_OUTPUT_TOKENS in attrs:
            ...
            attrs["gen_ai.usage.output_tokens"] = attrs[self._LLM_USAGE_COMPLETION_TOKENS]
        ...
        if self._LLM_USAGE_CACHE_READ_INPUT_TOKENS in attrs:
            attrs["gen_ai.usage.cache_read_input_tokens"] = attrs[self._LLM_USAGE_CACHE_READ_INPUT_TOKENS]
    ...
    def _get_message_keys_to_remove(self, attrs: MutableMapping[str, Any]) -> set:
        ...
        keys_to_remove = set()
        for key in attrs:
            if key.startswith(self._GENAI_PROMPT_PREFIX) or key.startswith(self._GENAI_COMPLETION_PREFIX):
    ...
    def _transform_messages(self, attrs: MutableMapping[str, Any]) -> None:
        ...
        input_messages = self._collect_indexed_attributes(attrs, self._GENAI_PROMPT_PREFIX)
        if input_messages:
        ...
        output_messages = self._collect_indexed_attributes(attrs, self._GENAI_COMPLETION_PREFIX)
        if output_messages:
    ...
    def _collect_indexed_attributes(self, attrs: MutableMapping[str, Any], prefix: str) -> Dict[int, Dict[str, Any]]:
        """
    ...
    def _structure_messages(self, indexed_messages: Dict[int, Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for index in sorted(indexed_messages.keys()):
            ...
            if "content" in message_data:
                ...
                structured_message["parts"].append({
                    "type": "text",
                    "content": content
                })
