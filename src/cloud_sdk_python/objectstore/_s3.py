...
from cloud_sdk_python.objectstore.utils import _normalize_host
...
class ObjectStoreClient:
    ...
    def __init__(self, creds_config: ObjectStoreBindingData, *, disable_ssl: bool = False) -> None:
    ...
    def _build_data_access_event(self, object_name: str, success: bool) -> DataAccessEvent:
    ...
    def put_object_from_bytes(self, name: str, data: bytes, content_type: str) -> None:
        ...
        try:
            ...
            raise ObjectOperationError(f"Failed to upload object '{name}': {e.code} - {e.message}") from e
        except Exception as e:
    ...
    def put_object(self, name: str, stream: BinaryIO, size: int, content_type: str) -> None:
    ...
    def put_object_from_file(self, name: str, file_path: str, content_type: str) -> None:
    ...
    def get_object(self, name: str) -> HTTPResponse:
        ...
        try:
            ...
            raise ObjectOperationError(f"Failed to download object '{name}': {e.code} - {e.message}") from e
        except Exception as e:
            get_err = e
            raise ObjectOperationError(f"Failed to download object '{name}': {e}") from e
        finally:
            # Log audit event
            try:
                self._audit_client.log(self._build_data_access_event(name, get_err is None))
            except Exception as e:
    ...
    def delete_object(self, name: str) -> None:
        ...
        try:
            ...
            if e.code != "NoSuchKey":
                raise ObjectOperationError(f"Failed to delete object '{name}': {e.code} - {e.message}") from e
    ...
    def list_objects(self, prefix: str) -> List[ObjectMetadata]:
        ...
        try:
            ...
            list_err = e
            raise ListObjectsError(f"Failed to list objects with prefix '{prefix}': {e.code} - {e.message}") from e
        except Exception as e:
            list_err = e
            raise ListObjectsError(f"Failed to list objects with prefix '{prefix}': {e}") from e
        finally:
            if prefix is None or prefix.strip() == "":
                # Audit log name can't be empty
                prefix = "/"
            try:
                self._audit_client.log(self._build_data_access_event(prefix, list_err is None))
            except Exception as e:
    ...
    def head_object(self, name: str) -> ObjectMetadata:
        ...
        try:
            ...
            if e.code == "NoSuchKey":
                raise ObjectNotFoundError(f"Object '{name}' not found") from e
            raise ObjectOperationError(f"Failed to get metadata for object '{name}': {e.code} - {e.message}") from e
        except Exception as e:
            head_err = e
            raise ObjectOperationError(f"Failed to get metadata for object '{name}': {e}") from e
        finally:
            try:
                self._audit_client.log(self._build_data_access_event(name, head_err is None))
            except Exception as e:
    ...
    def object_exists(self, name: str) -> bool:
        ...
        try:
            ...
            raise ObjectOperationError(f"Failed to check if object '{name}' exists: {e}") from e
