"""Generic OData v4 client shared across all DPI NG capabilities."""
...
class BaseODataClient(ABC):
    """Generic OData v4 transport layer for DPI NG capabilities.

    Manages a single ``requests.Session``, creates ``ODataService`` instances
    lazily per service endpoint, and caches entity classes per service.

    Subclasses must implement ``_get_entity_factories`` to register the
    OData entity factories specific to their capability.

    Example subclass::

        class _ConsentODataClient(BaseODataClient):
            def _get_entity_factories(self):
                from .entities.foo import _make_entities as foo_entities
                return {"fooServices": foo_entities}
    """

    def __init__(self, config: BaseCapabilityConfig) -> None:
        ...
        - Initializes an empty ODataService registry; individual instances are
          created lazily on first use via _get_service.

        Args:
            config: A validated class `BaseCapabilityConfig` (or subclass) instance.
        """
        logger.info("Invoked BaseODataClient.__init__")
        self._config = config
        ...
        self._factories: dict[str, Callable] = self._get_entity_factories()
        if config.tenant_id:
            self._session.headers.update({"x-tenant-id": config.tenant_id})
        logger.info("Exiting BaseODataClient.__init__")

    @abstractmethod
    def _get_entity_factories(self) -> dict[str, Callable]:
        """Return a mapping of service name to entity factory callable.

        Each factory is a ``_make_entities(svc: ODataService) -> tuple`` function
        defined in the capability's ``entities/`` package.

        Subclasses must implement this method to register their own entities.

        Returns:
            Dict mapping OData service endpoint names to factory callables.
        """
    ...
    def _get_service(self, service_name: str) -> ODataService:
        ...
        Args:
            service_name: The OData service endpoint name (e.g. ``"consentServices"``).

        Returns:
            The ODataService bound to the resolved service URL and shared session.
        """
        logger.info("Invoked BaseODataClient._get_service")
        ...
        logger.info("Exiting BaseODataClient._get_service")
    ...
    def get_entity_classes(self, service_name: str) -> tuple:
        ...
        """
        logger.info("Invoked BaseODataClient.get_entity_classes")
        ...
        logger.info("Exiting BaseODataClient.get_entity_classes")
    ...
    def query(self, service_name: str, entity_cls: type) -> Query:
        ...
        """
        logger.info("Invoked BaseODataClient.query")
        svc = self._get_service(service_name)
        result = svc.query(entity_cls)
        logger.info("Exiting BaseODataClient.query")
        return result
    ...
    def save(self, entity: Any) -> None:
        ...
        """
        logger.info("Invoked BaseODataClient.save")
        ...
        logger.info("Exiting BaseODataClient.save")
    ...
    def delete_entity(self, entity: Any) -> None:
        ...
        """
        logger.info("Invoked BaseODataClient.delete_entity")
        self._session.headers["If-Match"] = "*"
        try:
            entity.__odata_service__.delete(entity)
        finally:
            self._session.headers.pop("If-Match", None)
        logger.info("Exiting BaseODataClient.delete_entity")
    ...
    def call_action(
        ...
        """
        logger.info("Invoked BaseODataClient.call_action")
        ...
        if resp.status_code == 204 or not resp.content:
            logger.info("Exiting BaseODataClient.call_action — 204 No Content")
            return None
        result = resp.json()
        logger.info("Exiting BaseODataClient.call_action")
        return result
    ...
    def close(self) -> None:
        ...
        logger.info("Invoked BaseODataClient.close")
        self._session.close()
        logger.info("Exiting BaseODataClient.close")

    def __enter__(self) -> BaseODataClient:
    ...
    def _raise_for_status(resp: requests.Response) -> None:
        """Translate 4xx/5xx HTTP responses into typed ``DPINGError`` subclasses.
