...
```python
from sap_cloud_sdk.core.secret_resolver import SdkEnvVarResolver

resolver = SdkEnvVarResolver()  # base prefix: CLOUD_SDK_CFG
resolver = SdkEnvVarResolver("MY_APP_SECRETS")  # custom prefix
```
...
resolver = ChainedResolver([MountResolver(), SdkEnvVarResolver()])
...
configure(SdkConfig(resolver=ChainedResolver([VaultResolver(), SdkEnvVarResolver()])))
```

If `configure()` is never called, each module falls back to the default chain:
`ChainedResolver([MountResolver(), SdkEnvVarResolver()])`.
