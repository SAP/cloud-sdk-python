...
```python
from sap_cloud_sdk.dpi_ng.consent import (
    create_client,
    ConsentConfig,
...
```python
from sap_cloud_sdk.dpi_ng.consent import ConsentConfig, BearerTokenAuth
...
The consent client supports multi-tenancy, with the model depending on the
authentication strategy in use.

### How tenant identity is carried

| Auth strategy | Tenant identity source | `tenant_id` in config |
|---|---|---|
| `ClientCertificateAuth` | `x-tenant-id` request header | **Required** |
| `ClientCredentialsAuth` | Embedded in the OAuth2 token | Must not be set |
| `BearerTokenAuth` | Embedded in the bearer token | Must not be set |

For `ClientCertificateAuth`, the mTLS handshake does not carry a tenant claim,
so the service router needs the `x-tenant-id` header to dispatch requests to the
correct tenant. The SDK injects this header automatically from `tenant_id` at
client construction time and sends it on every request. Omitting it raises
`ValueError` immediately at `ConsentConfig` creation.

For token-based auth strategies, the tenant is already encoded in the token
itself (as the `zid` claim in XSUAA-issued tokens). Setting `tenant_id`
alongside these strategies raises `ValueError` to prevent accidental
double-specification.

`tenant_id` is a client-level setting, not a per-request parameter. To operate
against multiple tenants simultaneously, instantiate a separate client for each.
...
```python
from sap_cloud_sdk.dpi_ng.consent import CreateConsentRequest
...
```python
from sap_cloud_sdk.dpi_ng.consent import WithdrawConsentRequest
...
```python
texts = client.purposes.list_purpose_texts(filter="purposeId eq '<purpose-uuid>'")
```
...
text = client.purposes.update_purpose_text(
    ...
    body={
        ...
        "type_code": "01",
        "language_code": "de",
        "text": "Updated marketing email description.",
    },
)
```
...
```python
templates = client.templates.list_templates(filter="lifecycleStatusCode eq '2'")
```
...
```python
texts = client.templates.list_template_texts(filter="templateId eq '<template-uuid>'")
```
...
text = client.templates.update_template_text(
    ...
    body={
        ...
        "language_code": "de",
        "type_code": "52",
        "text": "Revised consent statement.",
    },
)
```
...
```python
rules = client.retention.list_rules(filter="lifecycleStatusCode eq '2'")
```
...
tp = client.configuration.update_third_party(
    ...
    {
        "third_party_name": "Analytics_Corp",
        "formatted_description": "Updated analytics provider description",
    },
)
...
dst = client.configuration.update_data_subject_type(
    ...
    {
        "data_subject_type_name": "Internal_Employee",
        "master_data_source_name": "SAP_SuccessFactors",
    },
)
...
app = client.configuration.update_application(
    "<application-uuid>",
    {
        "application_name": "HR_Portal_v2",
        "source_name": "CRM_System",
        "description": "HR portal version 2",
    },
)
...
source = client.configuration.update_master_data_source(
    ...
    {
        "master_data_source_name": "SAP_S4HANA_Cloud",
        "description": "SAP S/4HANA Cloud master data source",
    },
)
...
# Get
channel_type = client.configuration.get_outbound_channel_type(
...
channel_type = client.configuration.update_outbound_channel_type(
    "<outbound-channel-type-uuid>",
    {
        "outbound_channel_type_name": "Email_Newsletter",
        "description": "Email newsletter channel",
    },
)
