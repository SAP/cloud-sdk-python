...
def configured_consent_client(context: ConsentScenarioContext, live_client: ConsentClient) -> None:
...
def create_controller(context: ConsentScenarioContext, live_client: ConsentClient) -> None:
    ...
    print(f"[step 1] controller created: id={controller.controller_id} name={controller.controller_name}")
...
def create_application(context: ConsentScenarioContext, live_client: ConsentClient) -> None:
    ...
    print(f"[step 2] application created: id={app.application_id} name={app.application_name}")
...
def reuse_or_create_jurisdiction(context: ConsentScenarioContext, live_client: ConsentClient) -> None:
    existing = [
        j for j in live_client.configuration.list_jurisdictions()
...
def create_data_subject_type(context: ConsentScenarioContext, live_client: ConsentClient) -> None:
    ...
    print(f"[step 4] data subject type created: id={dst.data_subject_type_id} name={dst.data_subject_type_name}")


@then("the data subject type should have a valid id")
def data_subject_type_has_valid_id(context: ConsentScenarioContext) -> None:
    assert context.data_subject_type_id, "data_subject_type_id must be set after creation"


@when("I create a third party")
def create_third_party(context: ConsentScenarioContext, live_client: ConsentClient) -> None:
    ...
    print(f"[step 5] third party created: id={tp.third_party_id} name={tp.third_party_name}")
...
def create_purpose(context: ConsentScenarioContext, live_client: ConsentClient) -> None:
    ...
    print(f"[step 6] purpose created: id={purpose.purpose_id} name={purpose.purpose_name}")
...
def add_purpose_text(context: ConsentScenarioContext, live_client: ConsentClient) -> None:
    assert context.purpose_id, "purpose_id must be set (scenario 2 depends on scenario 1 passing)"
...
def activate_purpose(context: ConsentScenarioContext, live_client: ConsentClient) -> None:
...
def create_template(context: ConsentScenarioContext, live_client: ConsentClient) -> None:
...
def assign_third_party_recipient(context: ConsentScenarioContext, live_client: ConsentClient) -> None:
...
def assign_third_party_source(context: ConsentScenarioContext, live_client: ConsentClient) -> None:
    assert context.template_id, "template_id required"
    assert context.third_party_id, "third_party_id required"
    tppd = live_client.templates.create_third_party_pers_data({
        "template_id": context.template_id,
...
def activate_template(context: ConsentScenarioContext, live_client: ConsentClient) -> None:
...
def create_consent_record(context: ConsentScenarioContext, live_client: ConsentClient) -> None:
    ...
    print(f"[step 12] {len(consents)} consent record(s) returned: ids={context.consent_ids}")
