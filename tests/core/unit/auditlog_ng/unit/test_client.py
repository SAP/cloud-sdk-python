...
# ---------------------------------------------------------------------------
# Helpers shared by TestTraceSpanCorrelation
# ---------------------------------------------------------------------------

_TRACE_ID = 0xABCD1234ABCD1234ABCD1234ABCD1234
_SPAN_ID = 0x1234ABCD1234ABCD


def _make_real_client_with_exporter() -> tuple[AuditClient, InMemoryLogRecordExporter]:
    """Build an AuditClient backed by an InMemoryLogRecordExporter (no mocks)."""
    exporter = InMemoryLogRecordExporter()
    provider = LoggerProvider(
        resource=Resource.create(
            {
                "service.name": "test",
                "sap.ucl.deployment_id": "dep-1",
                "sap.ucl.system_namespace": "ns-1",
            }
        )
    )
    ...
    config = _make_config()
    # Bypass the real gRPC exporter and inject our in-memory provider directly.
    with patch("sap_cloud_sdk.core.auditlog_ng.client.GRPCLogExporter"), \
         patch("sap_cloud_sdk.core.auditlog_ng.client.LoggerProvider", return_value=provider):
        client = AuditClient(config)

    return client, exporter


def _make_valid_event(tenant_id: str = "tenant-123") -> pb.DataAccess:
    event = pb.DataAccess()
    event.common.tenant_id = tenant_id
    event.common.user_initiator_id = "user@example.com"
    event.common.timestamp.FromDatetime(datetime.now(timezone.utc))
    event.channel_type = "API"
    event.channel_id = "ch-001"
    event.object_type = "Resource"
    event.object_id = "obj-001"
    event.attribute = "attr-001"
    return event


class TestTraceSpanCorrelation:
    """Verify that trace_id and span_id from an active OTel span are
    automatically captured on emitted log records."""

    def test_active_span_correlates_trace_and_span_id(self):
        client, exporter = _make_real_client_with_exporter()
        ...
        assert log.trace_id == _TRACE_ID
        assert log.span_id == _SPAN_ID
        assert log.trace_flags == TraceFlags(TraceFlags.SAMPLED)

    def test_no_active_span_yields_zero_ids(self):
        client, exporter = _make_real_client_with_exporter()

        # Ensure no span is active (detach any inherited context).
        empty_ctx = context_api.Context()
        token = context_api.attach(empty_ctx)
        try:
            client.send(_make_valid_event())
        finally:
            context_api.detach(token)
    ...
    def test_log_record_carries_audit_attributes_alongside_trace_context(self):
        client, exporter = _make_real_client_with_exporter()
        ...
        token = context_api.attach(trace.set_span_in_context(NonRecordingSpan(span_ctx)))
        try:
            event_id = client.send(_make_valid_event())
        finally:
            context_api.detach(token)

        log = exporter.get_finished_logs()[0].log_record
        assert log.trace_id == _TRACE_ID
        assert log.span_id == _SPAN_ID
        assert log.attributes["cloudevents.event_id"] == event_id
        assert log.attributes["sap.tenancy.tenant_id"] == "tenant-123"
        assert log.attributes["sap.auditlogging.mime_type"] == "application/protobuf"
