...
class TestLocalDestinationClientLabels:
    ...
    def test_get_labels_returns_stored_labels(self, client):
        _write_store(client, {
            "instance": [{"name": "destA", "type": "HTTP", "labels": [{"key": "env", "values": ["prod"]}]}],
            "subaccount": [],
        })
        labels = client.get_destination_labels("destA", Level.SERVICE_INSTANCE)
        assert len(labels) == 1
    ...
    def test_get_labels_raises_404_for_missing_entity(self, client):
        with pytest.raises(HttpError) as exc_info:
            client.get_destination_labels("ghost", Level.SERVICE_INSTANCE)
        assert exc_info.value.status_code == 404
    ...
    def test_put_labels_raises_404_for_missing_entity(self, client):
        with pytest.raises(HttpError) as exc_info:
            client.update_destination_labels("ghost", [Label(key="env", values=["prod"])], Level.SERVICE_INSTANCE)
        assert exc_info.value.status_code == 404
    ...
    def test_patch_labels_raises_404_for_missing_entity(self, client):
        ...
        with pytest.raises(HttpError) as exc_info:
            client.patch_destination_labels("ghost", patch, Level.SERVICE_INSTANCE)
        assert exc_info.value.status_code == 404
