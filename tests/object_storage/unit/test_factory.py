...
class TestExplicitConfigRouting:
    def test_s3_config_dispatches_to_s3_client(self):
        with patch("sap_cloud_sdk.object_storage._factory.S3Client") as ctor:
            ...
            ctor.assert_called_once_with(S3_CONFIG)

    def test_azure_config_dispatches_to_azure_client(self):
        with patch("sap_cloud_sdk.object_storage._factory.AzureClient") as ctor:
            ...
            ctor.assert_called_once_with(AZURE_CONFIG)

    def test_gcs_config_dispatches_to_gcs_client(self):
        with patch("sap_cloud_sdk.object_storage._factory.GcsClient") as ctor:
            ...
            ctor.assert_called_once_with(GCS_CONFIG)
...
class TestAutoDetectPath:
    def test_default_instance_used_when_none_given(self):
        with (
            patch(
                "sap_cloud_sdk.object_storage._factory.read_binding_keys"
            ) as read,
            patch(
            ...
            patch(
                "sap_cloud_sdk.object_storage._factory.load_from_env_or_mount",
    ...
    def test_explicit_instance_propagated_to_resolution(self):
        with (
            patch(
                "sap_cloud_sdk.object_storage._factory.read_binding_keys"
            ) as read,
            patch(
                "sap_cloud_sdk.object_storage._factory.detect_provider",
                return_value=ObjectStoreProvider.GCS,
            ),
            patch(
                "sap_cloud_sdk.object_storage._factory.load_from_env_or_mount",
                return_value=GCS_CONFIG,
            ) as load,
            patch("sap_cloud_sdk.object_storage._factory.GcsClient") as ctor,
        ):
            create_client(instance="obj-1")
            read.assert_called_once_with("obj-1")
            load.assert_called_once_with(ObjectStoreProvider.GCS, "obj-1")
            ctor.assert_called_once_with(GCS_CONFIG)
